-- Phase 5.2: Activation + quota application from verified Razorpay webhooks
-- - Apply plan_version_id + roll billing period (cycle reset)
-- - Apply recurring structural add-ons (company_addon_subscriptions)
-- - Apply one-time variable top-up balances (company_addon_topups)
-- - Write canonical invoices and audit logs
-- Idempotency:
-- - webhook_events.event_id unique
-- - add unique (company_id, addon_id, checkout_session_id) for recurring add-ons
-- - provider_payment_id is deterministically derived even if Razorpay payment_id is missing

CREATE UNIQUE INDEX IF NOT EXISTS company_addon_subscriptions_checkout_unique
  ON public.company_addon_subscriptions (company_id, addon_id, checkout_session_id)
  ...
  IF p_event_type LIKE 'subscription.%' THEN
    ...
    IF v_subscription_id IS NOT NULL AND btrim(v_subscription_id) <> '' THEN
      ...
      IF v_company_id IS NOT NULL THEN
        ...
        -- Period timestamps (epoch seconds)
        v_period_start := NULL;
        v_period_end := NULL;
        BEGIN
        ...
        -- Upsert canonical company subscription.
        ...
        -- Apply recurring structural add-ons from the locked quote (checkout session).
        ...
        -- Reset base usage counters when a valid new period exists.
        ...
        -- Audit: subscription activated/updated.
        IF to_regclass('public.audit_logs') IS NOT NULL THEN
          INSERT INTO public.audit_logs (action, company_id, actor, status, metadata, created_at)
          VALUES (
            CASE WHEN v_subscription_status = 'active' THEN 'SUBSCRIPTION_ACTIVATED' ELSE 'SUBSCRIPTION_UPDATED' END,
            v_company_id,
            'razorpay',
            'success',
            jsonb_build_object(
    ...
    IF v_company_id IS NOT NULL AND v_invoice_id IS NOT NULL AND btrim(v_invoice_id) <> '' THEN
      ...
      IF to_regclass('public.audit_logs') IS NOT NULL THEN
        ...
        VALUES (
          ...
          'razorpay',
          'success',
          jsonb_build_object('invoice_id', v_invoice_id, 'status', v_invoice_status, 'event_id', p_event_id),
          v_now
        );
      END IF;
      ...
      IF FOUND THEN
        ...
        IF to_regclass('public.audit_logs') IS NOT NULL THEN
          ...
          VALUES (
            ...
            'razorpay',
            'success',
            jsonb_build_object('order_id', v_order_id, 'payment_id', v_payment_id, 'event_id', p_event_id),
            v_now
          );
        END IF;
