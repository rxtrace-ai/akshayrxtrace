-- ============================================================
-- PHASE 3: Entitlement priority hardening (Paid overrides Trial)
-- ============================================================
-- Phase 0 decision lock:
--   Paid plan > Trial > Free/None
--
-- This migration updates get_company_entitlement_snapshot to:
-- - Prefer PAID_ACTIVE when an active subscription exists, even if trial window is active.
-- - Support trial windows stored in Phase 1 columns (trial_start_at/trial_end_at) while remaining backward compatible
--   with legacy trial_started_at/trial_expires_at.
...
BEGIN
  ...
  -- Trial window: prefer Phase 1 columns when present, fallback to legacy fields.
  v_trial_start := COALESCE(v_company.trial_start_at, v_company.trial_started_at);
  v_trial_end := COALESCE(v_company.trial_end_at, v_company.trial_expires_at);
  ...
  -- Phase 0 priority: PAID overrides TRIAL.
  ...
  ELSIF v_trial_active THEN
    ...
    -- Trial defaults (kept as-is for backward compatibility with existing UI).
    v_base_unit := 5000;
    v_base_box := 500;
    v_base_carton := 100;
    v_base_pallet := 25;
  ...
  -- Structural add-ons (recurring) are stacked for seat/plant/handset if the tables exist.
  IF to_regclass('public.company_addon_subscriptions') IS NOT NULL THEN
    SELECT
  ...
  -- Topups/variable add-ons (credits)
  IF to_regclass('public.company_addon_topups') IS NOT NULL THEN
    SELECT
  ...
  v_remaining := jsonb_build_object(
    'unit', greatest((v_base_unit - v_usage_unit) + v_topup_unit, 0),
    'box', greatest((v_base_box - v_usage_box) + v_topup_box, 0),
    'carton', greatest((v_base_carton - v_usage_carton) + v_topup_carton, 0),
    'pallet', greatest((v_base_pallet - v_usage_pallet) + v_topup_pallet, 0),
