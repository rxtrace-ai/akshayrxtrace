...
declare
  ...
  v_resource text;
  v_amount integer;
  v_capacity_addon_id uuid;
  v_capacity_duration_days integer;
...
begin
  ...
  loop
    ...
    if v_resource in ('seats', 'plants', 'handsets') then
      ...
      v_capacity_addon_id := nullif(v_addon_line->>'addon_id', '')::uuid;
      v_capacity_duration_days := null;
      v_capacity_expires_at := v_period_end;

      if v_capacity_addon_id is not null then
        select duration_days
        into v_capacity_duration_days
        from public.add_ons
        where id = v_capacity_addon_id;
      end if;

      if v_capacity_duration_days in (30, 60, 90) then
        v_capacity_expires_at := least(v_period_end, v_period_start + make_interval(days => v_capacity_duration_days));
      end if;
    else
    ...
    insert into public.quota_allocations (
      ...
      v_resource,
      v_amount,
      case when v_resource in ('seats', 'plants', 'handsets') then v_capacity_expires_at else v_period_end end,
      p_quote_id,
      jsonb_build_object(
        'quote_id', p_quote_id,
        'payment_intent_id', v_intent.id,
        'addon_id', v_addon_line->>'addon_id',
        'duration_days', v_capacity_duration_days,
        'correlation_id', p_correlation_id
      )
    )
    on conflict (company_id, source_quote_id, quota_type, resource) do update
