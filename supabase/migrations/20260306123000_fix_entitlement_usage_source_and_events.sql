-- ============================================================
-- Fix: Entitlement usage source + usage_events logging
-- ============================================================
-- Goals:
-- 1) Make get_company_entitlement_snapshot() compute usage from usage_counters
--    (single authoritative source used by consume_entitlement()).
-- 2) Ensure consume_entitlement() writes a usage_events row (if the table exists)
--    so audit/event views remain accurate and counters can be derived.
--
-- Notes:
-- - This migration overrides existing functions by re-creating them.
...
BEGIN
  ...
  -- Usage is computed from usage_counters for the active period_start.
  -- Note: period_start is a billing-cycle key (trial window start for trial; current_period_start for paid).
  SELECT
  ...
  -- Handsets (company devices) must come from public.handsets (not public.handset).
  BEGIN
...
BEGIN
  ...
  -- Base quota consumption:
  -- - Always update usage_counters for the current period_start (authoritative quota state).
  -- - Optionally log usage_events for audit (if table exists).
