...
vi.mock("@/lib/erp/importSessions", () => ({
  beginErpImportSession: async () => {
    ...
    return {
      ...
      sessionId: "session-1",
      responseStatus: 200,
      result: { success: true, replay: true, imported: 3 },
    };
  },
...
describe("POST /api/erp/ingest/unit", () => {
  beforeEach(() => {
    mockState.userId = "user-1";
    mockState.sessionMode = "replay";
  });

  it("replays completed import sessions by idempotency key", async () => {
    const { POST } = await import("@/app/api/erp/ingest/unit/route");
    const req = new Request("http://localhost/api/erp/ingest/unit", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Idempotency-Key": "idem-unit-1",
      },
      body: JSON.stringify({ rows: [{ sku_code: "SKU1" }] }),
    });
  ...
  it("rejects concurrent imports with the same in-flight idempotency key", async () => {
